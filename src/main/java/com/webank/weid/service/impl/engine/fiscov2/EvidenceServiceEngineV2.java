...
public class EvidenceServiceEngineV2 extends BaseEngine implements EvidenceServiceEngine {
    ...
    public ResponseData<EvidenceInfo> getInfo(String hash) {
        ...
        try {
            ...
            logger.info(
                Thread.currentThread().getId() + "Outer cycle: getLatestRelatedBlock takes ms: " + (
                    System.currentTimeMillis() - timestamp) + ",blk:" + latestBlockNumber);
            ...
            logger.info(
                Thread.currentThread().getId() + "Outer cycle: resolveTransaction takes ms: " + (
        ...
        while (previousBlock != 0) {
            ...
            try {
                ...
                if (blockTransactionReceipts == null) {
                    ...
                    System.out.println(DataToolUtils.serialize(blockTransactionReceipts));
                    ...
                    if (blockTransactionReceipts != null
                        ...
                        .getTransactionReceipts().size() > 1) {
                        receiptsNode
                            .put(String.valueOf(currentBlockNumber), blockTransactionReceipts);
                    } else {
                        logger.info(Thread.currentThread().getId() + "current txn size <= 1, omit");
                    }
                    logger.info(Thread.currentThread().getId() + "batchGet takes ms: " + (
                        System.currentTimeMillis() - timestamp));
                } else {
                    logger.info(Thread.currentThread().getId() + "cache load takes ms: " + (
                        System.currentTimeMillis() - timestamp));
                }
            } catch (Exception e) {
                logger.error(
            ...
            try {
                ...
                logger.info(Thread.currentThread().getId() + "resolvelog takes ms: " + (
                    System.currentTimeMillis() - timestamp));
            } catch (Exception e) {
